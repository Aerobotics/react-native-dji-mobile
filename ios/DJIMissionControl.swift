...
class DJIMissionControlWrapper: NSObject {
  ...
  private func buildAircraftYawAction(_ parameters: NSDictionary) -> DJIAircraftYawAction? {
    let angle = parameters["angle"] as? Double
    let velocity = parameters["velocity"] as? Double
    let isAbsolute = parameters["isAbsolute"] as? Bool

    if (velocity != nil) {
      return DJIAircraftYawAction(relativeAngle: angle!, andAngularVelocity: velocity!)
    } else {
      return DJIAircraftYawAction(angle: angle!, andIsAbsolute: isAbsolute ?? true)
    }
  }
